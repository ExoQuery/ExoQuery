@file:PhasesDisabled(DisableablePhase.CrunchFlatJoins::class, DisableablePhase.SymbolicReduction::class)
package io.exoquery
...
class FlatUnitBugNoCrunchReq: GoldenSpecDynamic(GoldenQueryFile.Empty, Mode.ExoGoldenTest(), {
  ...
  "flatUnitBug - firstTwoFiltered - nocrunch" {
    val q = sql.select {
      val c = from(BCustomers.filter { it.status == "active" && it.region == "US" })
